...
extern "C" {
	...
	HMODULE LoadLibraryImm(const char* filename);

	FARPROC GetFromKernelImm(const char* process);

	FARPROC GetFromKernelLegacyImm(const wchar_t* process);

	void LogNormalImm(const char* text);

	void LogWarnImm(const char* text);

	void LogErrorImm(const char* text);

	void LogNoticeImm(const char* text);

	void LogSuccessImm(const char* text);
